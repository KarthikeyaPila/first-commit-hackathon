...
def test_tfidf_calibration_promotes_scores_at_or_above_040() -> None:
    ...
    promoted = score_pair(
        first, second, _source("first"), _source("second"),
        tfidf_similarity=0.64,
    )
    review = score_pair(
        first, second, _source("first"), _source("second"),
        tfidf_similarity=0.63,
    )

    assert promoted.score >= 0.40
    assert promoted.outcome == "MATCH"
    assert review.score < 0.40
