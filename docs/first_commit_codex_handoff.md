...
The deployed API includes health, processing, run status with detailed stage telemetry, state stories, story details, comparisons, and cached market context. Gold and silver are presented as INR per 10 grams. The hourly EventBridge schedule is enabled and invokes the processing Lambda. The next major work is the Sutradhar frontend: India map -> printing press -> animated real pipeline -> state-wise story output, followed by frontend hosting/polish. Do not restart the old prototype or embeddings plan unless explicitly requested.
