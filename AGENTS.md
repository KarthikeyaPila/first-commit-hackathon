...
- src/first_commit/clustering.py: global story graph clustering and state
  ...
  algorithm-version metadata. Headline TF-IDF artifacts are cached per snapshot. The interactive API uses a
  bounded 1,200-article, 4-neighbor preview budget; larger runs remain offline.
...
- MUST load the `aws-secrets-manager` skill first for any secret, credential,
  API key, token, or password task. MUST NOT call `secretsmanager get-secret-value`
  or `batch-get-secret-value`, and MUST NOT hit the Secrets Manager Agent daemon
  directly. MUST use `{{resolve:secretsmanager:secret-id:SecretString:json-key}}`
  with `asm-exec` so the secret resolves at runtime without entering context.
